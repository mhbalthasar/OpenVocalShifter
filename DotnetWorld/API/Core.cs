...
namespace DotnetWorld.API
{
    public class Core
    {
        ...
        public static string InitDllFile()
        {
            ...
            try
            {
                string srcAssembly = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), srcDll);
                string dstAssembly = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), dstDll);

                if (File.Exists(srcAssembly))
                {
                    File.Copy(srcAssembly, dstAssembly, true);
                }
            }
            catch {; }
            return "";
